...
//      echo "total:".$totalWithdrw+$totaltransfer;
   ...
   public function paginationLimit()
    ...
    public function my_level_team($userid)
    {
        $arrin = array($userid); // Initialize with the root user ID
        $ret = [];               // To store team levels
        $processed = [];         // To track already processed users (avoid circular references)
        $i = 1;                  // Start from level 1
    
        // Infinite loop that continues as long as there are downline members
        while (!empty($arrin)) {
            // Fetch downline members for the current level
            $alldown = User::select('id')->whereIn('sponsor', $arrin)->get()->toArray();
          
            // If there are new downline members
            if (!empty($alldown)) {
                // Get the IDs of the next level's members
                $arrin = array_column($alldown, 'id');
               
                // Filter out already processed users (to prevent cycles)
                $arrin = array_diff($arrin, $processed);
               
                // If there are still unprocessed users, store the current level's team
                if (!empty($arrin)) {
                    $ret[$i] = $arrin; // Store the current level's team
                    $processed = array_merge($processed, $arrin); // Add these users to processed
                    $i++; 
                              // Move to the next level
                } else {
                    // If no new members to process, break the loop
                    break;
                }
            } else {
                // No more downline members, exit loop
                break;
            }
        }
        
        // Flatten the multi-level array into a single-level array
        // $final = [];
        // if (!empty($ret)) {
